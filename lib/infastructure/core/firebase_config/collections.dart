...
class ArticlesCollection extends FirestoreCollection<Article> {
  ...
  @override
  CollectionReference<Article> get withConverter => firebaseFirestore
      .collection(path)
      .withConverter<Article>(
        fromFirestore: (snapshot, _) =>
            Article.fromJson(snapshot.data()!..addAll({'uid': snapshot.id})), // snapshot.data() is Firestore document data (as a Map<String, dynamic>) & Adds uid to article model
        toFirestore: (_, __) => {},
      );
}
